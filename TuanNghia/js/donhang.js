...
async function loadOrders(filterStatus = "all", searchKeyword = "") {
  ...
  try {
    ...
    for (const docSnap of snapshot.docs) {
      ...
      const totalText = new Intl.NumberFormat("vi-VN").format(totalPrice);
      ...
      row.innerHTML = `
        ...
        <td>
          <div style="display:flex; align-items:center; gap:8px; justify-content:center;">
            <img src="${
              data.thumbnail || ""
            }" alt="thumb" width="40" height="40" style="border-radius:6px;object-fit:cover;">
        ...
        <td>${totalText}</td>
        <td>${createdAt}</td>
        <td><span class="status-${data.status}">${getStatusText(
        data.status
      )}</span></td>
        <td>
    ...
    orderTableBody.innerHTML =
      "<tr><td colspan='7'>Không thể tải dữ liệu Firestore.</td></tr>";
  }
}
...
window.updateOrderStatus = async (orderId, newStatus) => {
  try {
    ...
    alert(
      `✅ Đã cập nhật đơn ${orderId.substring(0, 8)}... thành ${getStatusText(
        newStatus
      )}`
    );
