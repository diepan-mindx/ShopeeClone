...
async function loadUsers() {
  ...
  userTableBody.innerHTML =
    '<tr><td colspan="5">Đang tải người dùng...</td></tr>';
  try {
    const snapshot = await getDocs(collection(db, "users"));
    userTableBody.innerHTML = "";
    if (snapshot.empty) {
      userTableBody.innerHTML =
        '<tr><td colspan="5">Chưa có người dùng nào được lưu.</td></tr>';
      return;
    }
    ...
    snapshot.forEach((docSnap) => {
      ...
      row.innerHTML = `
                <td>${docSnap.id || "Chưa cập nhật"}</td>
                <td>${data.name || "N/A"}</td>
                <td class="role-cell">${data.email || "N/A"}</td>
                <td class="role-cell">${data.role || "user"}</td>
                <td class="role-cell">
                    <button class="btn btn-sm btn-primary" onclick="toggleAdmin('${id}', '${
        data.role || "user"
      }')">
